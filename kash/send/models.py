...
from kash.xlib.utils.utils import TransactionType, TransactionStatusEnum
...
class SendKash(BaseModel):
    ...
    recipients = models.ManyToManyField(
        "kash.UserProfile", related_name="kash_received"
    )
    initiator = models.ForeignKey(
        "kash.UserProfile", on_delete=models.CASCADE, related_name="kash_sent"
    )
    ...
    def fees(self):
        amount = (
            (self.amount * self.recipients.count())
    ...
    def pay(self, phone, gateway):
        ...
        new_contacts = [
            recipient for recipient in recipients if recipient not in contacts
        ]
    ...
    #     from kash.transaction.models import KashTransaction
...
def payout_recipients(sender, **kwargs):
    ...
    if (
        txn.content_type == send_kash_type
        ...
        if send_kash.recipients.count() == 1:
            ...
            transaction = send_to_recipient(
                recipient, send_kash, send_kash.amount.amount
            )
            if (
                transaction
                and transaction.status == TransactionStatusEnum.success.value
            ):
                send_kash.record_transaction(
                    recipient, send_kash.amount, txn, transaction
                )
            ...
            if send_kash.group_mode == SendKash.GroupMode.normal:
                ...
                for recipient in send_kash.recipients.all():
                    ...
                    if (
                        ...
                        send_kash.record_transaction(
                            recipient, amount, txn, transaction
                        )
                        send_kash.paid_recipients.add(recipient)
                        send_kash.save()
